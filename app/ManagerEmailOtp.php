...
class ManagerEmailOtp extends BaseModel
{
    protected $fillable = [
        'otp',
        'email',
    ...
    public function validator(array $data, $id = false, $isUpdate = false)
    {
        if ($isUpdate === true && !empty($id)) {
            $emailValidator = ['unique:manager_email_otps,email,' . $id];
        } else {
            $emailValidator = ['unique:manager_email_otps'];
        }
    ...
    public function updateOtp(int $id, array $data)
    {
        ...
        $getOtpInfo = $this->where('manager_id', $id)->first();
        if (empty($getOtpInfo)) {
        ...
        if ($update) {
            ...
            return ['isError' => 'false', 'message' => __('Manager email otp updated successfully !')];
        } else {
